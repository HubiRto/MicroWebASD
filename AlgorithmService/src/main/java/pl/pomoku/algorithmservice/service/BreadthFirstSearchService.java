...
import pl.pomoku.algorithmservice.repository.BreadthFirstSearchRepository;
import pl.pomoku.algorithmservice.utils.HashUtils;

@Service
@RequiredArgsConstructor
public class BreadthFirstSearchService {
    private final BreadthFirstSearchRepository repository;
    ...
    public FirstSearchOutput getResult(FirstSearchInput input) {
        ...
        return repository.findByHash(hash)
                .map(BreadthFirstSearchEntity::getOutput)
                .orElseGet(() -> {
                    FirstSearchOutput output = new DepthFirstSearch().calculate(input);
                    repository.save(new BreadthFirstSearchEntity(hash, input, output));
                    return output;
                });
    }
}
