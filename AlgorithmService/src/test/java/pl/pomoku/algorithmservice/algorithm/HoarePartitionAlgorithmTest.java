...
public class HoarePartitionAlgorithmTest {
    ...
    public void hoarePartitionResultTest1() {
        int[] arr = {3,12,10,0,6,14,19,16,11,1,9};
        ...
        assertFalse(Arrays.equals(step3.getArr(), new int[]{3,11,1,16,2}) && step3.getIndex() == 4);
        assertEquals(output.getDepth(), 3);
        assertTrue(Arrays.equals(step3.getArr(), new int[]{19,16,11,12,14}) && step3.getIndex() == 1);
    }

    @Test
    @DisplayName("Hoare Partition PJATK Result Test 2")
    public void hoarePartitionResultTest2() {
        int[] arr = {11,9,16,18,6,4,0,5,2,8,1};
        HoarePartitionOutput output = HoarePartition.calculate(arr, 9);
        HoarePartitionStep step2 = output.getSteps().get(1);

        assertFalse(Arrays.equals(step2.getArr(), new int[]{16,18,6,4,11,5,2,8,9}) && step2.getIndex() == 10);
